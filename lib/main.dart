...
class _WebViewExampleState extends State<WebViewExample> {
  ...
  void initState() {
    ...
    controller
      ...
      ..loadRequest(Uri.parse("https://sparrowsoftech.com/sspl/"));
  ...
  Future<void> _openCamera() async {
    try {
      ...
      // Show field data collection form first
      ...
      if (pickedFile != null) {
        await _saveImageToAppFolder(pickedFile, fieldData ?? {});
      } else {
        if (!mounted) return;
  ...
  Future<void> _saveImageToAppFolder(
    ...
    try {
      ...
      if (await destinationFile.exists()) {
        ...
        if (!mounted) return;
        _showImageSavedDialog(destinationFile);
      } else {
  ...
  void _showImageSavedDialog(File savedImage) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
...
/*class DataExportDialog extends StatefulWidget {
  // Fix: Add the key parameter to the constructor
  const DataExportDialog({super.key});
...
class _DataExportDialogState extends State<DataExportDialog> {
  ...
  Future<void> _exportData(String format) async {
    ...
    try {
      ...
      final fileName = 'gis_export_$timestamp';

      switch (format) {
        case 'kml':
          await GISExporter.exportToKML(photos, fileName);
          break;
        case 'geojson':
          await GISExporter.exportToGeoJSON(photos, fileName);
          break;
        case 'csv':
          await GISExporter.exportToCSV(photos, fileName);
          break;
      }

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Data exported as $fileName.$format')),
      );
      Navigator.of(context).pop();
    } catch (e) {
      if (!mounted) return;
              ...
              children: [
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () => _exportData('kml'),
                    child: const Text('Export as KML'),
                  ),
                ),
                const SizedBox(height: 8),
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text('Export as GeoJSON'),
                  ),
                ),
                const SizedBox(height: 8),
                SizedBox(
